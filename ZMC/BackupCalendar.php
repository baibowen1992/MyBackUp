...
public static function initReportCalendar(ZMC_Registry_MessageBox $pm)
{
	...
	switch($subnavInfo)
	{
		...
		case "DataIntegrity":
			$selectRangeSize=1;
			$GLOBALS['calMonthIsPassive'] = true;
			break;
...
public static function renderCalendar($pm)
{
	...
	while ($calMonth->hasMoreDays())
	{
		...
		for ($z=0; $z < 7;$z++)
		{
			...
			if (isset($BIDCalArray[$dayIndex]))
			{
				
				$numBID = $BIDCalArray[$dayIndex]->mGetNumItem();
				if ($numBID==1)
				{
					$cellClass .= "singleImage ";
					$singleStat=$BIDCalArray[$dayIndex]->mGetCumulativeStatus();
					$iconImage = ZMC_Report::mGetStatusIconImage($singleStat,false,$nextPrevMonth);
					$altStr = $BIDCalArray[$dayIndex]->mGetToolTip();
				}
				else 
				{
					$cellClass .= "multiImage ";
					$multiStat=$BIDCalArray[$dayIndex]->mGetCumulativeStatus();
					$iconImage = ZMC_Report::mGetStatusIconImage($multiStat,true,$nextPrevMonth);
					$altStr = $BIDCalArray[$dayIndex]->mGetToolTip();
				}
			}
	
			if ($isProgress)
				$iconImage = '/images/global/calendar/icon_calendar_progress.gif';

			if ($iconImage != "")
				$imgStr = "<img title='".$altStr."' src='$iconImage'>";
			...
			{
				if (isset($BIDCalArray[$dayIndex]))
					echo "<td class='".$cellClass."'><a href=$url>$day[0]$imgStr</a></td>\n";
				else 
					echo "<td class='".$cellClass."'>$day[0]</td>\n";
			}
			else 
				echo "<td class='".$cellClass."'>$day[0]</td>\n";
